...
namespace WorldTrackerProjector
{
    public static class SummaryViewProjectorFunction
    {
            ...
            CreateLeaseCollectionIfNotExists = true)]IReadOnlyList<Document> input, ILogger log, CancellationToken cancellationToken)
        {
            ...
            var projector = new SummaryViewProjector(new ProjectorDomainViewRepository());

            await projector.Project(events, cancellationToken);

            log.LogInformation($"{nameof(SummaryViewProjectorFunction)} processed {input.Count} documents");
        }
    }
}
